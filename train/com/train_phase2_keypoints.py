...
# ---------------------------------------------------------------------------
# Persistence baseline — uses CoM history (= keypoint[8, MidHip] ... no, just take last com_gt[t])
# But X is now keypoints not CoM. The "persistence" reference for the
# Phase 2 forecaster is still CoM(t+i) = CoM(t), so we compute it from
# Y_test by reading the predecessor frames from com_gt directly.
# ---------------------------------------------------------------------------

# CoM at t (the last frame of each sample's history) — we need to look it up
# from com_gt using the sample's frame index in meta.
...
# ---------------------------------------------------------------------------
# Phase 2 GRU model — same scaffold as Phase 1, only the input dim differs.
# ---------------------------------------------------------------------------
