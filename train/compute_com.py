...
Limitations documented:
  - Head (0.081 body mass) excluded: no valid distal head keypoint in this dataset.
    This biases CoM_z ~40 mm low in BOTH GT and pred, so the comparison is unaffected.
  - Hands (0.006 each) excluded: wrists only; combined effect < 1 cm on CoM.
  - Accounted mass fraction: 0.907 of total body mass.
  - No force-plate ground truth: comparison is pred-CoM vs GT-keypoint-derived CoM.

Run from inside train/:
    python compute_com.py
    python compute_com.py --out ./predictions/com/results.p
"""
...
parser.add_argument('--exp_dir',    type=str, default='./',
                    help='Directory containing ckpts/ (run from inside train/)')
parser.add_argument('--ckpt',       type=str, default='singlePerson_0.0001_10_best',
                    help='Checkpoint filename without .path.tar')
parser.add_argument('--test_dir',   type=str, default='./singlePerson_test/',
                    help='Test data path')
parser.add_argument('--window',     type=int, default=10)
parser.add_argument('--batch_size', type=int, default=32)
parser.add_argument('--out',        type=str, default='./predictions/com/com_results.p',
...
_TOTAL_MASS = sum(s[2] for s in SEGMENTS)   # 0.907
...
# Smoothness: frame-to-frame jump in mm
...
print(f'  NOTE: head+hands excluded (~9.3% mass) -> both GT and pred biased ~40 mm high equally')

# Sanity: CoM_x,y within carpet bounds [-100, 1800] mm
